...
# === ИГРА ГОНКИ (УЛУЧШЕННАЯ СТАБИЛЬНАЯ ВЕРСИЯ) ===
...
async def cmd_race(message: types.Message):
    ...
    # Регистрируем сессию гонки в БД (автоматически списывает ставку и защищает транзакцию)
    db.start_game(message.from_user.id, "race", bet)
    db.increment_games_played(message.from_user.id)
    ...
    # Создаем компактную разметку кнопок для 100% срабатывания
    builder = InlineKeyboardBuilder()
    for name, idx in animals:
        builder.row(types.InlineKeyboardButton(
            text=name,
            callback_data=f"race_choose:{idx}:{message.from_user.id}"
        ))
...
async def handle_race_choice(callback: types.CallbackQuery):
    try:
        ...
        # Получаем данные о сессии игры из БД
        game = db.get_active_game(owner_id)
        if not game or game["type"] != "race":
            await callback.answer("❌ Сессия гонки не найдена или уже завершена.", show_alert=True)
            try:
        ...
        # Моментально убираем анимацию загрузки кнопки
        ...
        db.update_game_level(owner_id, 1, 10.0 if won else 0.0, {})
        payout = db.finish_game(owner_id, won=won) # Безопасное начисление и удаление сессии
        ...
        if won:
            result_text = (
                ...
                f"💰 Ваш баланс: **{db.get_balance(owner_id):,}** коинов."
            )
        else:
            result_text = (
                f"💥 **ПРОИГРЫШ!**\n"
        ...
        try:
            ...
            logger.error(f"Ошибка вывода финиша: {e}")

    except Exception as e:
        logger.exception(f"Критическая ошибка в гонках: {e}")
